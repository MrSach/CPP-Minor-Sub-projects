/*
	Program
		Multithreading with Mutex - Smart Pointer for C array of Thread objects
	...
	Description
		...
		This is an attempt to use smart pointers with C arrays that contain threads.
...
int main (int argc, char * * argv)
{
	...
	*/
	...
	/*
		Declare "t_arr" as a unique smart pointer that contains the "std::thread" object with 10 elements.
		Note: The c++17 standard supports unique pointers here but not shared pointers!
		Initialise array and the elements with the std::make_unique function.
	*/
	std::unique_ptr <std::thread []> t_arr [10]
	{
		std::make_unique <std::thread []> (10)
	};
	...
	/*
		...
		Remember that as "t_arr" is a smart pointer, the ->get() member method is necessary in order to fetch the contained variable.
		As this variable is a C array, the square bracket notation automatically dereferences the value, so no asterisk is needed here.
	*/
	for (size_t i = 0; i < 10; ++i)
	{
		t_arr->get() [i] = std::thread (threadable () );
	}
